...
export default function Home() {
  ...
  useEffect(() => {
    // Kayan yazı için interval
    const interval = setInterval(() => {
      setCurrent((prev) => (prev + 1) % phrases.length);
    }, 2500);

    // Firebase'den kuaförleri çek
      ...
      <div className="overflow-x-auto flex gap-4 px-4 pb-6">
        {recommendedPhotos.map((photo, i) => (
          ...
          >
            <img
              src={photo}
              alt={`Önerilen Kuaför ${i + 1}`}
              className="h-32 w-full object-cover"
            />
            <div className="p-2">
              <h3 className="text-sm font-bold">Krispi King {i + 1}</h3>
